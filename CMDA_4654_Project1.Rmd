...
## The question we are looking to answer is, how does 3-Point Percentage effect a teams Offensive Efficiency, 
 
#pairs(merged_df[20:30], pch = 16, lower.panel = NULL)
library(fANCOVA)
fit.loess.opt <- with(merged_df, 
                 loess.as(`3PA`, OE, degree = 1, criterion = c("aicc", "gcv")[2], user.span = NULL, plot = F))
summary(fit.loess.opt)

fit.loess.opt <- with(merged_df, 
                 loess.as(`3PA`, OE, degree = 2, criterion = c("aicc", "gcv")[2], user.span = NULL, plot = F))
summary(fit.loess.opt)
