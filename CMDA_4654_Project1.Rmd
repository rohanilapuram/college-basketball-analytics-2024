...
ridge.plot <- plot_ly(x = X_train[,"OE"], y = X_train[,"SOS"], z = y_train, type = "scatter3d", mode = "markers", marker = list(color = "blue", size = 5)) %>%
  layout(title = "3D Scatter Plot of Training Data",
         scene = list(xaxis = list(title = "OE"), 
                      yaxis = list(title = "SOS"), 
                      zaxis = list(title = "Wins"))) 
  # Add the plane using the coefficients

ridge.plot <- add_trace(p= ridge.plot,
                        z = predicted_wins,
                        x = X_train[,"OE"],
                        y = X_train[,"SOS"],
                        )
...
```{r}
# Calculate the plane equation using the coefficients
intercept <- -61.7454572
coef_OE <- 0.7459537
coef_SOS <- -0.2537968
...
# Add the plane using the predicted wins
ridge.plot <- add_surface(p = ridge.plot, z = matrix(predicted_wins, nrow = length(unique(X_train[,"OE"])), ncol = length(unique(X_train[,"SOS"]))),
                          x = unique(X_train[,"OE"]),
                          y = unique(X_train[,"SOS"]),
                          )
