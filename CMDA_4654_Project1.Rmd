...
```{r}
library(fANCOVA)
fit.loess.opt <- with(merged_df, 
                 loess.as(`3P_Percentage`, OE, degree = 1, criterion = c("aicc", "gcv")[2], user.span = NULL, plot = F))
summary(fit.loess.opt)

library(fANCOVA)
fit.loess.opt <- with(merged_df, 
                 loess.as(`3P_Percentage`, OE, degree = 2, criterion = c("aicc", "gcv")[2], user.span = NULL, plot = F))
summary(fit.loess.opt)

ggplot(merged_df, aes(x = `3P_Percentage`, y = OE)) + geom_point() +
  geom_smooth(method = "loess", span = 0.4843714, se = F, method.args = list(degree = 1), aes(color = "degree 1")) + 
  geom_smooth(method = "loess", span = 0.6612838, se = F, method.args = list(degree = 2), aes(color = "degree 2")) + 
...
X_train <- train_data[, c(  "SOS", "Rk", "OE", "DE", "Conference Wins")]
y_train <- train_data$NCAA_Tourney
X_test <- test_data[, c( "SOS", "Rk", "OE", "DE", "Conference Wins")]
y_test <- test_data$NCAA_Tourney
...
ggplot(data = accuracy_df, aes(x = K, y = Accuracy)) +
  geom_line() +
  geom_point() +
  labs(title = "Accuracies vs. K Values",
       x = "K Value",
       y = "Accuracy") +
  theme_minimal()
```
...
```{r}
pca_result <- prcomp(train_data[, c("SOS", "Rk", "OE", "DE", "Conference Wins")], scale. = TRUE)

# Convert PCA results to a data frame
pca_df <- as.data.frame(pca_result$x)
pca_df$NCAA_Tourney <- as.factor(y_train)
pca_df$Team <- train_data$Team  # Assuming "Team" column contains team names

# Create interactive PCA graph with plotly
pca_plot <- plot_ly(data = pca_df, x = ~PC1, y = ~PC2, color = ~NCAA_Tourney, text = ~Team, type = "scatter", mode = "markers",
